...
class PluginInfo
{
    ...
    {
        // Each of these checks is broken out so that a plugin author can get specific error message if the plugin
        // is malformed.
        if (!$this->plugin_dir) {
        ...
        if (!is_readable($this->plugin_dir)) {
            throw new TerminusException(
                'The directory "{dir}" is not readable',
                ['dir' => $this->plugin_dir,]
            );
        }
        ...
        if (!$info) {
            throw new TerminusException(
                'The file "{file}" does not contain valid JSON',
                ['file' => $composer_json,]
            );
        }
        ...
        if ($this->hasAutoload($info)) {
            ...
            foreach ($namespaces as $namespace) {
                if (substr($namespace, -1) != '\\') {
                    throw new TerminusException(
                        'The namespace "{namespace}" in the composer.json autoload psr-4 section must end '
                            . 'with a namespace separator. Should be "{correct}"',
