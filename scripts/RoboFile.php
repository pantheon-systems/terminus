...
class RoboFile extends \Robo\Tasks
{
  ...
  public function sniff(
    ...
    $result = $this->taskExec(self::PROJECT_ROOT . "vendor/bin/phpcs --standard=PSR2 {$strict} {$file}")->run();
    if (!$result->wasSuccessful()) {
      if (!$options['autofix']) {
        $options['autofix'] = $this->confirm('Would you like to run phpcbf to fix the reported errors?');
      }
      if ($options['autofix']) {
        $result = $this->taskExec(self::PROJECT_ROOT . "vendor/bin/phpcbf --standard=PSR2 {$file}")->run();
      }
    }
    return $result;
  }
