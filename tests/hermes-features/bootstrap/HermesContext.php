...
namespace Pantheon\Terminus\HermesTests;
...
/**
 * Class HermesContext
 * Features context for Behat feature testing against Hermes
 * @package Pantheon\Terminus\HermesTests
 */
class HermesContext implements Context
{
    public $cliroot = '';
    ...
    const DEFAULT_BACKUP_DAY = 'Monday';
    const DEFAULT_UPSTREAM = 'e8fe8550-1ab9-4964-8838-2b9abdccf4bf';
    ...
    {
        if (!getenv('TERMINUS_MACHINE_TOKEN') || !getenv('TERMINUS_USER_ID') || !getenv('TERMINUS_USERNAME')) {
            throw new \Exception('Env vars TERMINUS_MACHINE_TOKEN, TERMINUS_USER_ID, and TERMINUS_USERNAME must all be defined.');
        }
        ...
        $this->parameters['machine_token'] = getenv('TERMINUS_MACHINE_TOKEN');
        $this->parameters['user_id'] = getenv('TERMINUS_USER_ID');
        $this->parameters['username'] = getenv('TERMINUS_USERNAME');
        $this->start_time      = time();
        $this->connection_info = [
            'host' => $parameters['host'],
            'machine_token' => getenv('TERMINUS_MACHINE_TOKEN'),
        ...
        $this->session_path = $this->cache_dir . '/session';
        $this->plugin_dir = $this->fixtures_dir . '/plugins';
        $this->plugin_dir_name = self::DEFAULT_PLUGIN_DIR_NAME;
    }

    /**
     * Ensures a site of the given name exists and creates it if it doesn't
     *
     * @Then /^a site named "([^"]*)" exists$/
     *
     * @param string $site Name of site to ensure exists
     * @return bool True if the site already exists
     * @throws \Exception If the named site does not exist and belongs to the user
     */
    public function aSiteNamedExists($site)
    {
        try {
            $this->iRun("terminus site:lookup $site");
        } catch (TerminusException $e) {
            throw new \Exception("A site named $site belonging to the user does not exist.");
        }
        return true;
    }
     ...
     *
     * @Given /^a site named "([^"]*)" already exists$/
     *
     * @param string $site Name of site to ensure exists
     * @return bool Always true
     */
    public function aSiteNamedAlreadyExists($site)
    {
        try {
            $this->aSiteNamedExists($site);
        } catch (\Exception $e) {
            $this->iCreateSiteNamed(self::DEFAULT_UPSTREAM, $site);
        }
        return true;
    }

    /**
     * Ensures a site of the given name DNE
     *
     * @Then /^a site named "([^"]*)" does not exist$/
     *
     * @param string $site Name of site to ensure DNE
     * @return bool True if the site DNE
     */
    public function aSiteNamedDoesNotExist($site)
    {
        $this->iRun("terminus site:lookup $site");
        $this->iShouldGetTheError("Could not locate a site your user may access identified by $site.");
        return true;
    }

    /**
     * Ensures a site of the given name does not exist and erases it if it does
     *
     * @Given /^a site named "([^"]*)" does not already exist$/
     *
     * @param string $site Name of site to ensure DNE
     * @return bool Always true
     */
    public function aSiteNamedDoesNotAlreadyExist($site)
    {
        try {
            $this->aSiteNamedDoesNotExist($site);
        } catch (\Exception $e) {
     ...
     * @return bool Always true, else errs
     */
    ...
    /**
     * Tests the user for being logged in
     *
     * @return bool If the user is authenticated
     */
    public function iAmAuthenticated()
    {
        if (!$this->getFilesystem()->exists($this->session_path)) {
     ...
     * @throws \Exception If user is not logged in as the given user
     */
    public function iAmAuthenticatedAs($user_id)
    {
        $this->iAmAuthenticated();
        $session_data = json_decode(file_get_contents($this->session_path));
        $test_user_id = $this->replacePlaceholders($user_id);
        if ($session_data->user_id !== $test_user_id) {
            throw new \Exception(
                "User is not authenticated as $test_user_id. Authenticated user is {$session_data->user_id}."
            );
        }
        return true;
    }
    ...
    /**
     * Tests the user for not being logged in
     * @Then /^I should not be authenticated$/
     *
     * @return bool True if logged not in
     * @throws \Exception If user is logged in
     */
    ...
    {
        $this->getFilesystem()->remove($this->cache_token_dir);
        return true;
    }

    /**
     * Checks to ensure no machine tokens are saved
     * @Given I should have no saved machine tokens
     *
     * @return bool True if there are no saved machine tokens
     * @throws \Exception If machine tokens are saved to the token cache dir
     */
    public function iShouldHaveNoSavedMachineTokens()
    {
        $finder = $this->getFinder();
        $finder->files()->in($this->cache_token_dir);
        if ($finder->hasResults()) {
            $number_of_tokens = count($finder);
            throw new \Exception("$number_of_tokens machine tokens are saved.");
        }
        return true;
    }
    ...
    /**
     * Logs in user with the Behat config machine token
    ...
    {
        $this->getFilesystem()->remove($this->session_path);
    }
     ...
     * @return bool $i_have_this True if $string exists in output
     * @throws Exception
     */
     ...
     * Checks the output for the given string that it is an error with the given string
     *
     * @param string $string Content which ought not be in the output
     * @return bool True if $string exists in output
     * @throws Exception
     */
    public function iShouldGetTheError($string)
    {
        return $this->iShouldGet("[error]  $string");
    }
    ...
    {
        return $this->iShouldGet("[notice] $string");
    }
     ...
     * @return bool true if $headers exists in output
     *
     * @throws \Exception
     */
     ...
     * @return bool true if all of the rows are present in the output
     * @throws \Exception
     */
     ...
     * @return bool true if all of the given number of rows are present
     * @throws \Exception
     */
     ...
     * @return bool True if a $string exists in output
     * @throws Exception
      */
     ...
     * @return bool True if a $string does not exist in output
      */
     ...
     *
     * @return bool True if new backup exists
     */
     ...
     *
     * @param string $string Content which ought not be in the output
     * @return bool True if $string does not exist in output
     */
     ...
     * @return bool True if $member does exists in output
     */
     ...
     * @return bool True if $member does not exist in output
     */
     ...
     * @return bool $result True if $nededle was found in $haystack
     */
    private function checkResult($needle, $haystack)
    {
        $needle = $this->replacePlaceholders($needle);
        $haystack = $this->replacePlaceholders($haystack);
    ...
    /**
     * @Given the :env environment of :site does not already have backups scheduled
     */
    public function theEnvironmentOfDoesNotAlreadyHaveBackupsScheduled($env, $site)
    {
        $this->iRun("terminus backup:automatic:disable $site.$env");
    }

    /**
     * @Then the :env environment of :site has backups scheduled
     * @Then the :env environment of :site has backups scheduled for :day
     */
    public function theEnvironmentOfHasBackupsScheduledFor($env, $site, $day = null)
    {
        $this->iRun(
            "terminus backup:automatic:info $site.$env"
        );
        if ($day === null) {
            $this->iShouldNotGet('Backups are not currently scheduled to be run.');
        } else {
            $this->iShouldGet($day);
        }
    }
    ...
    /**
     * @Given the :env environment of :site already has backups scheduled
     * @Given the :env environment of :site already has backups scheduled for :day
     */
    public function theEnvironmentOfAlreadyHasBackupsScheduledFor($env, $site, $day = null)
    {
        $this->iRun(
            "terminus backup:automatic:enable $site.$env --day=" . $day || self::DEFAULT_BACKUP_DAY
        );
    }

    /**
     * @Then the :env environment of :site does not have backups scheduled
     */
    public function theEnvironmentOfDoesNotHaveBackupsScheduled($env, $site)
    {
        $this->iRun("terminus backup:automatic:info $site.$env");
        $this->iShouldGetTheNotice('Backups are not currently scheduled to be run.');
    }
