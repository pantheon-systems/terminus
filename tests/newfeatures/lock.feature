Feature: Locking and unlocking a site
  ...
  Scenario: Locking and Unlocking an environment
    When I run "terminus lock:add --site=[[test_site_name]] --env=dev --username=pantheon --password=password"
    Then I should get:
    """
    ...
    When I run "terminus lock:rm --site=[[test_site_name]] --env=dev"
    Then I should get:
    """
  ...
  Scenario: Get lock info for an environment
    When I run "terminus lock:info --site=[[test_site_name]] --env=dev"
    Then I should get:
    """
    Locked
    """
