...
class FeatureContext implements Context {
  ...
  */
  ...
  public function iAddHostnameToTheEnvironmentOf($hostname, $env, $site) {
    $this->iRun(
      "terminus domain:add $hostname --site=$site --env=$env"
    );
  }
  ...
  public function iAttachTheInstrument($uuid, $site) {
    $this->iRun(
      "terminus payment-method:set $uuid --site=$site"
    );
  }
  ...
  public function iCommitChanges($env, $site, $message) {
    $this->iRun(
      "terminus env:commit $env --site=$site --message="
      . '"' . $message . '" --yes'
    );
  }
  ...
  public function iCreateSiteNamed($upstream, $name, $org = false) {
    ...
    $this->iRun(
      "terminus site:create $name --label=$name --upstream=\"$upstream\" $append_org"
    );
  }
  ...
  public function iCreateMultidevEnv($multidev, $env, $site) {
    $this->iRun(
      "terminus multidev:create --site=$site --to-env=$multidev --from-env=$env"
    );
  }
  ...
  public function iDeployTheEnvironmentOf($env, $from, $site, $message) {
    $this->iRun(
      "terminus env:deploy 
      --site=$site --to-env=$env --from-env=$from --note=$note"
    );
  }
  ...
  public function iGetInfoForTheEnvironmentOf($env, $site) {
    $return = json_decode(
      $this->iRun(
        "terminus env:info $env --site=$site --env=$env --format=json"
      )
    );
    return $return;
  }
  ...
  public function iGetInfoForTheSite($site) {
    $return = json_decode(
      $this->iRun("terminus site:info $site --format=json")
    );
    return $return;
  }
  ...
  public function iMakeBackupElementsOfTheEnvironment($elements, $env, $site) {
    $this->iRun(
      "terminus backup:create --site=$site --env=$env --element=$elements"
    );
  }
  ...
  public function iRemoveFromTheTeamOn($email, $site) {
    $this->iRun(
      "terminus site:team:remove --site=$site --member=$email"
    );
  }
