Feature: Tagging organizational sites
  ...
  Scenario: Adding a tag
    When I run "[[executable]] tag:add [[test_site_name]] '[[organization_name]]' testtag"
    Then I should get: "[[organization_name]] has tagged [[test_site_name]] with testtag."

  @vcr tag-list.yml
  Scenario: Listing a site's tags
    When I run "[[executable]] tag:list [[test_site_name]] '[[organization_name]]'"
    Then I should get: "- testtag"

  @vcr tag-remove.yml
  Scenario: Removing a tag
    When I run "[[executable]] tag:remove [[test_site_name]] '[[organization_name]]' testtag"
