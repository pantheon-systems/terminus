Feature: Set a Backup Schedule for a Site
  ...
  Scenario: Set the backup schedule for an environment with a TTL
    When I run "terminus backup:automatic:enable [[test_site_name]].test --day=Tue --keep-for=180"
    Then I should get "Backup schedule successfully set."
    When I run "terminus backup:automatic:info [[test_site_name]].test --field=expiry"
